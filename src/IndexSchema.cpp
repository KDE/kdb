...
IndexSchema::IndexSchema(TableSchema *tableSchema)
        ...
        , Object(Predicate::IndexObjectType)
...
// : FieldList(static_cast<const FieldList&>(idx))//fields are not owned by IndexSchema object
        : FieldList(false)//fields are not owned by IndexSchema object
        , Object(static_cast<const Object&>(idx))
...
//Qt 4 m_master_owned_rels.setAutoDelete(true); //rels at the master-side are owned
    ...
    foreach(Field *f, idx.m_fields) {
        ...
        if (!parentTableField) {
            PreWarn << "cannot find field" << f->name() << "in parentTable. Empty index will be created!";
            FieldList::clear();
            break;
        }
...
bool IndexSchema::isForeignKey() const
{
...
PREDICATE_EXPORT QDebug operator<<(QDebug dbg, const Predicate::IndexSchema& index)
{
    ...
    dbg.space() << static_cast<const Object&>(index) << '\n';

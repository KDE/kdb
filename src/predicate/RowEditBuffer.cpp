...
{
    if (!m_dbBuffer) {
        PreWarn << "RowEditBuffer::at(QueryColumnInfo&): not db-aware buffer!" << endl;
        return 0;
    }
...
{
    if (!m_simpleBuffer) {
        PreWarn << "RowEditBuffer::at(Field&): this is db-aware buffer!" << endl;
        return 0;
    }
...
{
    if (isDBAware()) {
        PreDbg << "RowEditBuffer type=DB-AWARE, " << m_dbBuffer->count() << " items" << endl;
    ...
    PreDbg << "RowEditBuffer type=SIMPLE, " << m_simpleBuffer->count() << " items" << endl;
